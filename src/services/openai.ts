...
class OpenAIService {
  private async makeRequest(messages: Array<{ role: string; content: string }>): Promise<string | null> {
    try {
      ...
      console.log('🔷 OpenAI: Response received:', response ? response.substring(0, 200) + '...' : 'null');
      return response;
    } catch (error) {
  ...
  async getTradeRecommendation(
    ...
    try {
      console.log('🔷 OpenAI: Getting trade recommendation...');
      const response = await this.makeRequest(messages);
      if (!response) {
        console.log('🔷 OpenAI: No response received from API');
        return null;
      }
      
      console.log('🔷 OpenAI: Raw response received:', response.substring(0, 300) + '...');
      
      // Extract JSON from the response
      const jsonMatch = response.match(/\{[\s\S]*\}/);
      if (jsonMatch) {
        console.log('🔷 OpenAI: JSON extracted successfully');
        try {
          ...
          console.error('🔷 OpenAI: Raw JSON string:', jsonMatch[0]);
          return null;
        }
      }
      
      console.error('🔷 OpenAI: Failed to extract JSON from response:', response);
      return null;
    } catch (error) {
  ...
  async analyzeStock(stockData: StockData): Promise<StockAnalysis | null> {
    ...
    try {
      ...
      console.error('OpenAI stock analysis error:', error);
      return null;
    }
  }
  ...
  async getMarketInsights(symbols: string[]): Promise<string | null> {
    ...
    try {
      ...
      console.error('OpenAI market insights error:', error);
      return null;
    }
  }
  ...
  async explainTrade(
    ...
    try {
      ...
      console.error('OpenAI trade explanation error:', error);
      return null;
    }
  }
}
